...
{
    public TemporaryFolder gwtProjectDir = new TemporaryFolder();

    [Test]
    public void testCompilesWithGwt()
    {
    ...
    private void addFiles(JarOutputStream jar, string _base, File f)
    {
        File[]
        files = f.listFiles();
        if (files == null) {
        ...
        for (File subF : files) {
            if (subF.isDirectory())
            {
                addFiles(jar, entryName(_base, subF), subF);
            }
            else
            {
    ...
    private void addFile(JarOutputStream jar, string _base, File f)
    {
        ZipEntry e = new ZipEntry(entryName(_base, f));
        jar.putNextEntry(e);
        ByteStreams.copy(new FileInputStream(f), jar);
    }

    private static string entryName(string _base, File f)
    {
        return _base.isEmpty() ? f.getName() : _base + "/" + f.getName();
    }
}
